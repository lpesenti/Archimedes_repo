...
def quantile_plot(filexml, Data_path, network, sensor, location, channel, tstart, Twindow, Overlap, verbose,
    ...
    # DAY
    q1_values_day = daytime_df.groupby(['frequency'])['psd_value'].quantile(q1)
    q2_quantile_day = daytime_df.groupby(['frequency'])['psd_value'].quantile(q2)
    q3_quantile_day = daytime_df.groupby(['frequency'])['psd_value'].quantile(q3)

    print('\t\tNIGHTTIME DATA')

    # NIGHT
    q1_values_night = nighttime_df.groupby(['frequency'])['psd_value'].quantile(q1)
    q2_quantile_night = nighttime_df.groupby(['frequency'])['psd_value'].quantile(q2)
    q3_quantile_night = nighttime_df.groupby(['frequency'])['psd_value'].quantile(q3)

    # Frequencies are the indeces of the DataFrame because of groupby()
    frequency_array = q1_values_day.index.to_numpy()
    ...
    # DAY
    q1_values_day_array = q1_values_day.to_numpy()
    q2_quantile_day_array = q2_quantile_day.to_numpy()
    q3_quantile_day_array = q3_quantile_day.to_numpy()

    print('\t\tNIGHTTIME DATA')

    # NIGHT
    q1_values_night_array = q1_values_night.to_numpy()
    ...
    if save_npz:
        ...
        np.savez(r'C:\Users\Arc_MDC\Documents\ET\P2\data.npz', frequency=frequency_array, q1_d=q1_values_day_array,
                 q2_d=q2_quantile_day_array, q3_d=q3_quantile_day_array, q1_n=q1_values_night_array,
                 q2_n=q2_quantile_night_array, q3_n=q3_quantile_night_array)
...
def new_quantile_plot(filexml, Data_path, network, sensor, location, channel, tstart, Twindow, Overlap, verbose,
            ...
            for n in range(0, M):
                ...
                if tr.get_gaps() == [] and len(tr) > 0:
                    ...
                    if tr1.stats.npts == Num:
                        ...
                        v[k] = 0
                if np.isnan(v[k]):
                    psd_values_db = np.tile(v[k], np.size(f))
                else:
                    pass
    ...
    if save_npz:
        print('Saving data to .npz file')
        np.savez(r'C:\Users\Arc_MDC\Documents\ET\P2\data_ALL.npz', frequency=frequency_array, q1=q1_array,
    ...
    if xscale == 'linear' or xscale == 'both':
        ...
        ax1.plot(frequency_array, q1_array, linewidth=2, label='{0}%'.format(q1 * 100))
        ax1.plot(frequency_array, q2_array, linewidth=2, label='{0}%'.format(q2 * 100))
        ax1.plot(frequency_array, q3_array, linewidth=2, label='{0}%'.format(q3 * 100))
    ...
    if xscale == 'log' or xscale == 'both':
        ...
        ax2.plot(frequency_array, q1_array, linewidth=2, label='{0}%'.format(q1 * 100))
        ax2.plot(frequency_array, q2_array, linewidth=2, label='{0}%'.format(q2 * 100))
        ax2.plot(frequency_array, q3_array, linewidth=2, label='{0}%'.format(q3 * 100))
        ...
        fig2.savefig(
            img_path + r'HD\{0}\{0}{2}_QuantilePlot-{3}_{4}_{1}_{5}_log.pdf'.format(sensor, channel, location,
                                                                                    ...
                                                                                    stopdate.strftime(
                                                                                        '%d-%b-%Y'), unit),
            dpi=1200)
        fig1.savefig(img_path + r'{0}\{0}{2}_QuantilePlot_{1}-{3}_{4}_{5}.png'.format(sensor, channel, location,
                                                                                      startdate.strftime(
                                                                                          '%d-%b-%Y'),
                                                                                      stopdate.strftime(
                                                                                          '%d-%b-%Y'), unit),
                     dpi=300)
        fig1.savefig(img_path + r'{0}\{0}{2}_QuantilePlot_{1}-{3}_{4}_{5}.pdf'.format(sensor, channel, location,
                                                                                      startdate.strftime(
                                                                                          '%d-%b-%Y'),
                                                                                      stopdate.strftime(
                                                                                          '%d-%b-%Y'), unit),
