...
lineList = [line.rstrip('\n') for line in open('C:\Archimedes\Dati\listafile.txt')]
...
# with open(r"C:\Users\Arc_MDC\Documents\Optical_Level\Data\SCI_OL_21-07-08_1125.lvm") as fobj:
#with open(r"C:\Archimedes\Dati\SosEnattos_Data_20220120\SCI_22-01-20_1501.lvm") as fobj:
...
ax0.plot(xtimex, c0, label="itf", linewidth=2)
#ax0.plot(xtimex, c2, label="Dx", linewidth=2)
#ax0.plot(xtimex, c1, label="Dy", linewidth=2)
...
s0, _ = mlab.psd(c3, NFFT=Num, Fs=fs, detrend="linear")  # ,noverlap=NOL)
s0 = s0[1:]
s0 = np.sqrt(s0)
#s1, _ = mlab.psd(c1, NFFT=Num, Fs=fs, detrend="linear")  # ,noverlap=NOL)
#s1 = s1[1:]
#s1 = np.sqrt(s1)
# s1=np.sqrt(s1)*1.4e-7*5.765
#s2, _ = mlab.psd(c2, NFFT=Num, Fs=fs, detrend="linear")  # ,noverlap=NOL)
#s2 = s2[1:]
#s2 = np.sqrt(s2)

### VIRGO ###
# x, y = np.loadtxt('/Users/drozza/Documents/UNISS/Archimedes/Archimedes_at_Virgo_201907/VirgoData_Jul2019.txt',
...
#ax1.plot(f, s2, label="Dx", linewidth=2)
#ax1.plot(f, s1, label="Dy", linewidth=2)  # Error-Signal")
ax1.plot(f, s0, label="Error-Signal", linewidth=2)
